...
const Usermanager = () => {
    ...
    const getData = async (page = activepage) => {
        setVisibale(true)
        let path = apiUrl.userList + '?page=' + `${page}`;
    ...
    const handleSearching = async (page = activepage) => {
        setVisibale(true);
        let path = apiUrl.userList + '?page=' + `${page}` + '&start_date=' + `${startdate == '' ? '' : moment(startdate).format('YYYY-MM-DD')}` + '&end_date=' + `${enddate == '' ? '' : moment(enddate).format('YYYY-MM-DD')}` + '&status=' + `${serachstatus}` + '&keyword=' + `${keywords}`;
        const fr = await Helper.get(path, token);
        const res = await fr.response.json();
        if (fr.status === 200) {
            if (res.status) {
        ...
        <div className="animated fadeIn loader-outer">
            ...
            <Row>
                <Col>
                    <Card>
                        <CardHeader>
                            <Link to="/addmanager" className="btn btn-primary float-right">
                                <i className="fa fa-plus"> Add New User</i>
                            </Link>
                            <i className="fa fa-list"></i> User Manager List
                      </CardHeader>
                        <CardBody>
                                    ...
                                    return (
                                        <tr key={key}>
                                            ...
                                            <td>
                                                ...
                                                <Link to={{ pathname: "/editmanager", state: item }} title="Edit" className="btn btn-info btn-sm text-white mr-1 "><i className="fa fa-pencil"></i></Link>
                        ...
                        <Row>
                            ...
                            <Col md={6}>
                                <FormGroup>
                                    ...
                                    <div> <img src={viewdata.image ? process.env.REACT_APP_API_BASE_URL + viewdata.image : require('../../assets/img/no.jpg')} alt="" width="150" /></div>
                                </FormGroup>
                            </Col>
                        </Row>
                    </Form>
